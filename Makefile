...
FLAGS_LINKER					+= -Wl,-u,vfprintf -lprintf_min
FLAGS_LINKER					+= -Wl,--defsym=__bad_rodata_start=0
FLAGS_LINKER					+= -Wl,--defsym=__bad_rodata_end=0
FLAGS_LINKER					+= -Wl,--gc-sections
FLAGS_LINKER					+= -Wl,--strip-all
FLAGS_LINKER					+= -Wl,-u,_exit
FLAGS_LINKER					+= -Wl,-u,exit -lc
FLAGS_LINKER					+= -Wl,-Map=$(BUILD_DIR)/$(BUILD_NAME).map,--cref
