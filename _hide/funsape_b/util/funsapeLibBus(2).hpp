...
#define __FUNSAPE_LIB_BUS_HPP 2407
...
#elif __FUNSAPE_LIB_GLOBAL_DEFINES_HPP != __FUNSAPE_LIB_BUS_HPP
...
#elif __FUNSAPE_LIB_DEBUG_HPP != __FUNSAPE_LIB_BUS_HPP
...
#elif __FUNSAPE_LIB_GPIO_HPP != __FUNSAPE_LIB_BUS_HPP
...
public:
    ...
    enum class BusType {
        ...
        AOSONG_SINGLE_BUS = 1, //!< Aosong Single Bus interface (bit-bang)
        DALLAS_ONE_WIRE = 2, //!< Dallas 1-Wire interface (bit-bang)
        PARALLEL_HD44780 = 3, //!< Parallel HD44780 controller interface (bit-bang)
        PARALLEL_SIMPLE = 4, //!< Parallel+clock generic interface (bit-bang)
        SPI = 5, //!< Serial Peripheral Interface
        TWI = 6, //!< Two Wire Interface (I2C)
        UART = 7, //!< Universal Asynchronous Receiver Transmitter
        USART = 8 //!< Universal Synchronous/Asynchronous Receiver Transmitter
    };
...
public:
    ...
    //     //////////////     ONE-WIRE PROTOCOL METHODS    //////////////     //
    //!
    //! \brief          Sets the device ROM code (64-bit address).
    //! \details        This function sets the device ROM Code for 1-wire
    //!                     communication.
    //! \param[in]      romCode_p       64-bit ROM Code
    //! \return         bool_t          True on success / False on failure
    //!
    virtual bool_t setDevice(
        cuint64_t romCode_p)
    {
        // Mark passage for debugging purpose
        debugMark("Bus::setDevice(cuint64_t)", DEBUG_BUS);
